...
export default function NotificationSettings() {
  ...
  return <><section className="settings-card"><p className="eyebrow">Notification settings</p><h2>Choose your in-app alerts</h2><p className="settings-copy">These preferences apply to the TalentXpanse notification center. Security emails and account-recovery emails are always sent when needed.</p><form className="notification-preferences" onSubmit={save}>{options.map(([key, title, detail]) => <label key={key}><input type="checkbox" checked={preferences[key]} onChange={(event) => setPreferences({ ...preferences, [key]: event.target.checked })} /><span><b>{title}</b><small>{detail}</small></span></label>)}{error && <p className="form-notice">{error}</p>}{notice && <p className="form-notice">{notice}</p>}<button disabled={busy} className="button button-primary">{busy ? 'Saving…' : 'Save preferences'}</button></form></section><section className="settings-card settings-note"><h2>What stays on</h2><p>Password recovery, email verification, and important account-security emails are not controlled here.</p></section></>
}
