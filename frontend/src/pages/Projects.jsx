...
export function ProjectDetailScreen() {
  ...
  const closeProject = (event) => {
    event.preventDefault()
    run(() => api.post(`/contracts/${id}/close`, { reason: closeReason.trim() }), 'Project closed. The project history remains available to both partners.').then(() => {
      setCloseReason('')
      setCloseOpen(false)
    })
  }
