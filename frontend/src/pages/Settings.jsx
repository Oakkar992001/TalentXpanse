...
export default function SettingsScreen({ section = 'information' }) {
  ...
  const saveInformation = async (event) => {
    ...
    try { const { data } = await api.put('/account-settings', { name }); setAccount(data.data); await refreshUser(); setDirty(false); setNotice('Your information was saved.') } catch (requestError) { setError(errorMessage(requestError)) } finally { setBusy(false) }
  }
  const resendVerification = async () => {
    setBusy(true); setError(''); setNotice('')
    try { const { data } = await api.post('/email/verification-notification'); setNotice(data.message) } catch (requestError) { setError(errorMessage(requestError)) } finally { setBusy(false) }
  }
  const activate = async (role) => {
    setBusy(true); setError('')
    try { await switchRole(role); navigate(`/dashboard?role=${role}`) } catch (requestError) { setError(errorMessage(requestError)) } finally { setBusy(false) }
  }
  const addWorkspace = async (role) => {
    setBusy(true); setError('')
    try { await addRole(role); navigate(`/workspace-setup?role=${role}`) } catch (requestError) { setError(errorMessage(requestError)) } finally { setBusy(false) }
  }
  ...
  return <section className="settings-page"><header><p className="eyebrow">Settings</p><h1>{section === 'information' ? 'My information' : section === 'account' ? 'Account settings' : section === 'security' ? 'Password and security' : section === 'notifications' ? 'Notification settings' : 'Membership and credits'}</h1><p>Keep your account details and marketplace workspace accurate.</p></header><div className="settings-layout"><SettingsSidebar /><main>
    ...
    {!account ? <p className="settings-loading">Loading account settings…</p> : section === 'information' ? <>
      <form className="settings-card" onSubmit={saveInformation}><div className="settings-card-heading"><div><p className="eyebrow">Contact information</p><h2>Your account</h2></div><span className={`status-pill ${account.account_status}`}>{account.account_status}</span></div>
      ...
      <section className="settings-card settings-note"><h2>Account security</h2><p>Email verification is active. Password changes and identity verification will be added before live payments are introduced.</p></section>
      ...
      <section className="settings-card settings-note"><h2>Privacy and account deletion</h2><p>These controls are intentionally unavailable until exports, payment history, and support procedures are ready.</p></section>
    </> : <section className="settings-card credits-settings"><p className="eyebrow">Freelancer workspace</p><h2>Proposal Credits</h2>{credits ? <><strong>{credits.balance}</strong><span>available credits</span><p>{credits.monthly_allowance || 20} credits are granted each month. Credit costs are shown before you submit a proposal.</p><Link className="button button-outline" to="/jobs">Find freelance jobs</Link></> : <p className="settings-loading">Loading credit balance…</p>}</section>}
  </main></div></section>
}
