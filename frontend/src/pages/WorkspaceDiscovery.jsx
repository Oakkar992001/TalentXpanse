...
export default function SearchResultsScreen() {
  ...
  useEffect(() => { if (user) loadSaved().catch(() => {}) }, [user?.id])
  useEffect(() => {
  ...
  const submit = (event) => {
    ...
    ;['category', 'skill', 'budget_type', 'experience_level', 'min_budget', 'max_budget', 'location', 'min_rate', 'max_rate', 'availability'].forEach((key) => { if (data.get(key)) next[key] = data.get(key) })
    setParams(next)
  }
