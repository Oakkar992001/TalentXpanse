...
export function AuthScreen({ mode }) {
  ...
  return <section className="auth-page"><div className="auth-panel"><Link className="brand auth-brand" to="/">Talent<span>Xpanse</span></Link><div className="auth-card"><p className="eyebrow">{isLogin ? 'Welcome back' : 'Create your account'}</p><h1>{isLogin ? 'Continue your journey.' : 'How would you like to use TalentXpanse?'}</h1><p className="auth-intro">{isLogin ? 'Sign in to manage your work and opportunities.' : 'Start with one path today. You can add the other profile later.'}</p>
...
export function DashboardScreen() {
  ...
  return <div className="dashboard-content"><header className="dashboard-header"><div><p className="eyebrow">{isClient ? 'Client' : 'Freelancer'} workspace</p><h1>Good morning, {user.name}</h1><p>{isClient ? 'Review your jobs and the people ready to help.' : 'Find the next project that moves your career forward.'}</p></div>{isClient ? <button className="button button-primary" onClick={() => setShowForm(!showForm)}>+ Post a job</button> : <Link className="button button-outline" to="/jobs">Find work</Link>}</header>{error && <Notice>{error}</Notice>}
