...
export default function FreelancerPublicProfile() {
  ...
  return <section className="public-profile">
    <div className="marketplace-detail-actions"><Link className="marketplace-back-link" to={searchPath}><span aria-hidden="true">←</span> Back to search</Link><div>{freelancer.id && <MarketplaceSaveButton kind="talent" targetId={freelancer.id} />} {freelancer.id && <MarketplaceReportButton targetType="freelancer" targetId={freelancer.id} />}</div></div>
    <header><span className="public-avatar">{profile.profile_photo_url ? <img src={profile.profile_photo_url} alt="" /> : profile.name?.slice(0, 1)}</span><div><p className="eyebrow">Available freelancer</p><h1>{profile.name}</h1><h2>{freelancer.title || 'Freelancer'}</h2><p>{freelancer.location || 'Myanmar'} | {freelancer.hourly_rate ? `Ks ${Number(freelancer.hourly_rate).toLocaleString()} / hr` : 'Rate shared in proposal'}{freelancer.experience_level && <> | {experienceLabels[freelancer.experience_level]}</>}</p></div></header>
